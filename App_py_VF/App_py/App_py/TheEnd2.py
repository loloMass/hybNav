...
def limit(graphdata):
    ...
    for i, point in enumerate(xy):  # On parcourt les points de xy d'indice i
        if (point[1] > 5000.0 and point[
            1] < 15000.0):  # seuils pour la veleur de la lumière lors de la l'entrée dans le building
            print(i, point)
            l[o] = i
            o = o + 1
            # print(l2[0])
...
graphGps = plt.plot(longilist[0:longueur - 1], latlist[0:longueur2 - 1],
                    label="latitude=f(longitude)")  # tracer la courbe
...
def deltaInDistGrandeListe(longitudeListe, latitudeListe):
    ...
    for j in range(len(latitudeListe)):
    ...
    for i, index in enumerate(taille2):
    ...
    for j in range(0, len(listeB)):
        if len(listeB[j]) != 0:
            moylisteB.append(moyenne(listeB[j], len(listeB[j])))
    for i in range(0, len(moylisteB) - 1):
        if moylisteB[i] < 1 and moylisteB[i + 1] < 1 and moylisteB[i + 2] < 1 and moylisteB[i + 3] < 1:
...
def detectionPic(listeOriginale, k, diviseur):
    ...
    for k1 in range(0, len(realIndicePics) - 1):
        ...
        elif realIndicePics[k1 + 1] - realIndicePics[k1] > 5 and listeOriginale[realIndicePics[k1]] < listeOriginale[
            realIndicePics[k1 + 1]]:
...
def deltaInDistance(longitudeListe, latitudeListe):
    deltaIndist = []
    ptPert = []
    for j in range(1, len(latitudeListe)):
        deltaIndist.append(6378 * 1000 * acos(
            sin((latitudeListe[0] * pi) / 180) * sin((latitudeListe[j] * pi) / 180) +
            cos((latitudeListe[0] * pi) / 180) * cos((latitudeListe[j] * pi) / 180) *
            cos((longitudeListe[j] * pi) / 180 - (longitudeListe[0] * pi) / 180)))
    """for j in range(0,len(deltaIndist)-1):
        if deltaIndist[j+1]<deltaIndist[j]:
            ptPert.append(j)"""
...
def compToGPS(lon, lat, xComp, tabAcce, biglon, biglat):
    ...
    ptTransition = deltaInDistGrandeListe(biglon, biglat)
    pasTransition = []
    ...
    plt.plot(lon[0: ptTransition], lat[0:  ptTransition], "g",
             label="latitude jusqu'au point de perturbation =f(longitude jusqu'au point de perturbation", linewidth=2)
...
def valeur(bigxComplist, bigyComplist, bigzComplist, biglat, biglon, biglux, xComp, tabAcce, lat, lon):
    ...
    for j in range(len(biglat)):
        ...
        if deltaLat[j] == 0 and deltaLat[j] == deltaLat[-1] == 0 and deltaLat[
                    j - 1] != 0:  # and lux[j][i]>lux[j][i-1] :
            stopPoint = j
    ...
    for j in range(1, len(moyDeltaInLat)):
        ...
        if moyDeltaInLat[j] < 0 and moyDeltaInLon[j] > 0 and moyDeltaInLat[j + 1] == 0:
            ...
            disturbancePoint = deltaInDistGrandeListe(biglon, biglat)
        else:
            disturbancePoint = deltaInDistGrandeListe(biglon, biglat)
...
def ecartType(graphdata, latList, longiList):
    ...
    a = []
    b = []
    latIntervalles = intervalles(latList[0:-1], 300,
                                 50)  # jusqu'à -1 pour récupérer latlist jusqu'au dernier élément de type float
    ...
    for i, elt in enumerate(latIntervalles):
        if (longiIntervalles[i][-1] - longiIntervalles[i][0]) != 0:  # On élimine les cas de division par zéro
            ...
            newLongiValues.append(longiIntervalles[
                                      i])  # La nouvelle liste de valeurs longitudinales de même taille que la liste des coefficients a et b
            newLatValues.append(latIntervalles[
                                    i])  # La nouvelle liste de valeurs de latitude de même taille que la liste des coefficients a et b
