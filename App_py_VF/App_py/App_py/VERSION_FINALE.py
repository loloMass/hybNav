...
def seuil(L):  # fonction pour déterminer les points de luxlist pendant l'entrée au niveau de la porte
    ...
    for i in range(0, len(L) - 1):
            if (L[i-1] - L[i ]) > 1000 : #and elt[i + 1] < 1000:
                 b.append (L[i -2])
    return b[-1]
#Recherche du point d'entrée de la porte
...
def reaLimitOutIn(graphdata, luxliste):
    ...
    oI_index = pointOutIn(latlist, longueur2)#point d'entrée de la porte
    inter = intervalles(luxliste, 300, 50)
    b = seuil(luxliste[0:-1])

    graphListe = luxliste[luxliste.index(b):oI_index]
    for nbr in graphListe:
        c = graphListe.count(nbr)
        if c > 1:
            for i in range(c - 1):
                graphListe.remove(nbr)
