...
def ecartType(graphdata,liste):
  ...
  #_______ calculd'écart type

    moyInterLat=[]
    ecartTypeListe=[]

    for i in range (0,len(liste)-1):  # On parcourt les points de xy d'indice i
        l2.append(liste[i])
    listeIntervalles = intervalles(l2, 300, 50)

    for i in range(0,len(listeIntervalles)-1):
        moyInterLat.append(moyenne(listeIntervalles[i], len(listeIntervalles[i])))

    for i, elt1 in enumerate(moyInterLat):
        j = 0
        acc = 0
        while j< len(listeIntervalles[i]):
            acc+=(listeIntervalles[i][j]-moyInterLat[i])**2
            j+=1
            #print ("acc",acc)
        variance= acc / len(listeIntervalles[i])
        ecartTypeValues=sqrt(variance)
        ecartTypeListe.append(ecartTypeValues)


    #print("écart type liste",ecartTypeListe)
    print("moyenne", moyInterLat)
    return ecartTypeListe
...
y=[]
k=ecartType(graphGps, longilist)
print("k",k)
a=(n[-1]-n[0])/(k[-1]-k[0])
b=n[0]-a*k[0]
