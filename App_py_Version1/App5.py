...
def tempsList(liste):
    ...
    return tps
"""echelleLat=[]
for x in range (0,len(latlist)-1):
    x=x*111.11
    echelleLat.append(x)"""
...
# _______**********************************************Sans conversion des coordonnées latitude et longitude
    for i, elt in enumerate(latIntervalles):
        if (longiIntervalles[i][-1] - longiIntervalles[i][0]) != 0:  #On élimine les cas de division par zéro
            a1.append((latIntervalles[i][-1] -latIntervalles[i][0])  / (longiIntervalles[i][-1] - longiIntervalles[i][0]))
            b1.append(latIntervalles[i][0] - a1[i] * longiIntervalles[i][0])

            newLongiValues1.append(longiIntervalles[i]) # La nouvelle liste de valeurs longitudinales de même taille que la liste des coefficients a et b
            newLatValues1.append(latIntervalles[i])  # La nouvelle liste de valeurs de latitude de même taille que la liste des coefficients a et b



    y1=axPlusb(newLongiValues1,a1,b1) #y= a*x+b avec x la liste des valeurs de  longitude ici newLongiValues


    for i in range(0, len(newLongiValues1)):
        plt.plot(newLongiValues1[i], y1[i]) #Affichage de y en fonction de newLongiValues y= a* newLongiValues + b
        plt.plot(newLongiValues1[i], newLatValues1[i],'r', linewidth=2.5)
    ...
    ecart1 = projection(newLongiValues1, newLatValues1, a1, b1)
    print("écart1", ecart1)

# _______**********************************************conversion en m des coordonnées latitude et longitude

    latliste2km = latlist2kmeter(latIntervalles)
    longilist2km = longilist2kmeter(latIntervalles)

    # _______recherche de a et b
    for i, elt in enumerate(latliste2km):
        if (longilist2km[i][-1] - longilist2km[i][0]) != 0:  #On élimine les cas de division par zéro
            a.append((latliste2km[i][-1] -latliste2km[i][0])  / (longilist2km[i][-1] - longilist2km[i][0]))
            b.append(latliste2km[i][0] - a[i] * longilist2km[i][0])

            newLongiValues.append(longilist2km[i]) # La nouvelle liste de valeurs longitudinales de même taille que la liste des coefficients a et b
            newLatValues.append(latliste2km[i])  # La nouvelle liste de valeurs de latitude de même taille que la liste des coefficients a et b

    # _______tracé des droites d'équation y= a*x+b
    ...
    for i in range(0, len(newLongiValues)):
        ...
        #plt.plot(longilist2km[i], latliste2km[i], 'r',linewidth=2.5)
    plt.xlabel('longitude')  # nom de l'axe x
    plt.ylabel('latitude')  # nom de l'axe y
    plt.legend();
    plt.show()
    ...
    for i, elt1 in enumerate(y):
        j = 0
        acc = 0
        while j < len(newLatValues[i]):

            acc += (newLatValues[i][j] - y[i][j]) ** 2
            j += 1
            # print ("acc",acc)
        variance = acc / len(newLatValues[i])
        ecartTypeValues = sqrt(variance)
        ecartTypeLat.append(ecartTypeValues)

    #print ("ecartType",ecartTypeLat)

    #print("azi", azimuth(latliste2km, longilist2km))
...
ecartType(graphGps, latlist,longilist)
#print("n",n)
...
print ("r",xComplistCalibree )
print(xComplist)
print ("l",len(xComplistCalibree))
...
print("ko",len(longilist))
print ("ik",len(latlist))